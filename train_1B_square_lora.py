...
def log_validation(vae, text_encoder, tokenizer, clip_image_encoder, transformer3d, config, args, accelerator, weight_dtype,
    ...
    with torch.no_grad():
        with torch.autocast("cuda", dtype=weight_dtype):
            ...
            text_prompt = "A short-haired woman with black hair, wearing a necklace and a white sports jacket, presents a professional look under bright, even lighting with a TV interview/live broadcast feel. Framed in a half-body, chest-up medium shot, her hands rest on a table with a blue desk mat. She looks into the camera while giving a live lecture. The background is a green screen."
...
def main():
    ...
    # transformer3d.train() # 这里不能训练，因为lora_network已经训练了
    ...
    transformer3d, lora_network, optimizer, train_square_dataloader, lr_scheduler = accelerator.prepare(transformer3d, lora_network, optimizer, train_square_dataloader, lr_scheduler)
        ...
        for step, batch in enumerate(train_square_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                with torch.no_grad():
                    try:
                        # 添加调试信息
                        if vocal_input_values.nelement() == 0:
                            raise ValueError("vocal_input_values is empty")
                        if torch.isnan(vocal_input_values).any():
                            raise ValueError("vocal_input_values contains NaN")
                        if torch.isinf(vocal_input_values).any():
                            raise ValueError("vocal_input_values contains Inf")
                        
                        vocal_embeddings = wav2vec(vocal_input_values).last_hidden_state
                    except Exception as e:
                        audio_path = batch["audio_path"]
                        print(f"Error processing audio {audio_path}: {str(e)}")
                        print(f"vocal_input_values shape: {vocal_input_values.shape if vocal_input_values is not None else 'None'}")
                        print(f"vocal_input_values device: {vocal_input_values.device if vocal_input_values is not None else 'None'}")
                        print(f"vocal_input_values dtype: {vocal_input_values.dtype if vocal_input_values is not None else 'None'}")
                        vocal_embeddings = torch.zeros([vocal_input_values.size(0), 168, 768], device=vocal_input_values.device if vocal_input_values is not None else accelerator.device)
