...
if __name__ == "__main__":
    ...
    print(f"Root folder path: {image_folder}")
    
    # 遍历整个文件夹结构，寻找所有的images子文件夹
    for root, dirs, files in os.walk(image_folder):
        # 只处理名为'images'的文件夹
        if os.path.basename(root) == 'images':
            print(f"Processing images folder: {root}")
            
            # 创建对应的face_masks文件夹
            face_masks_path = root.replace('images', 'face_masks')
            if not os.path.exists(face_masks_path):
                os.makedirs(face_masks_path)
                print(f"Folder created: {face_masks_path}")
            else:
                print(f"Folder already exists: {face_masks_path}")
            
            # 处理该images文件夹中的所有png文件
            for file in files:
                if file.endswith('.png'):
                    file_path = os.path.join(root, file)
                    face_save_path = os.path.join(face_masks_path, file)
